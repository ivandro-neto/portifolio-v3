...
const ExperienceCard: React.FC<ICardsProps> = ({
  period,
  role,
  seniority,
  company,
  ...
  return (
    <div className="flex w-full h-auto justify-end items-center p-4 hover:bg-backgroundlight rounded-md transition-all">
      <div className="flex flex-col border-l-2 border-zinc-600 pl-4">
        <span className="text-offtext text-sm font-semibold group-hover:text-zinc-50 xl:text-base">
          {period}
        </span>
        <h3 className="text-lg font-semibold group-hover:text-zinc-50 xl:text-xl">
          {seniority} {role} <span className="text-offtext">• {company}</span>
        </h3>
        <p className="text-offtext text-sm mt-2 px-2 py-1 text-justify group-hover:text-zinc-50 xl:text-base">
          {description}
        </p>
